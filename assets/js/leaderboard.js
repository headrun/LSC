(function($) {
  ...
  function loadnavdefaults(){
	  ...
	  $('.points').html('Total '+measurement_dimension+' : '+leaderboardapp.initialData.points);
	  
  }
  
  function loadnav(data){
	  loadnavdefaults();
	  $('.points').html('Total '+measurement_dimension+' : '+data.points);
  ...
  function updateLeaderboardData(data){
	  console.log(data);
	  $('.table').empty();
	  if(territory_to_manager=='ACTIVE'){
	  	var temp="Manager";
	  }
	  if(territory_to_manager=='INACTIVE'){
	  	var temp="Territory";
	  }
	  var leaderboardheading="<thead>"+
							 ...
							 "<td class='name'><h4 class='trdata'>"+temp+"</h4></td>"+
							 "<td class='name'><h4 class='text-right trdata'>"+measurement_dimension+"</h4></td>"+
							 "</tr>"+
							 "</thead>";
...
if('serviceWorker' in navigator) {
    navigator.serviceWorker
             .register('/LSC/service-worker.js')
