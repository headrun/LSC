    ...
    switch(cardData[i].objective_type){
      ...
      case "TARGET":
             ...
             // $('.'+cardData[i].id).attr('onclick',"window.location.assign(apiurl+'/dashboard/leaderboard');");
              ...
              var tempsize=cardData[i].target_ach_percentage;
              if(tempsize > 100){tempsize=100;}
              $('.'+cardData[i].id).children('.card-block').children('.row').last().children('.col-lg-8').children('.progress').children('.progress-bar').css('width',+tempsize+'%');
