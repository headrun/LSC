...
<style type="text/css">
  path{
    ...
    /*fill: rgb(227, 233, 244) !important;*/
...
<div class="row" style="width: 100%">
     <div class = "col-lg-9 col-md-9 col-sm-9 col-xs-9" id="cont" style="width: 900px;height: 800px;" ></div>
     <div class = "col-lg-3 col-md-3 col-sm-3 col-xs-3" style="">
        <label>Select State : </label><br>
        <select id="selectDrop" class="btn btn-primary dropdown-toggle optionsForStates">
        </select>
        <br>
        <br>
        <p>Select From and To Airports for Distance</p>
        <label>From :</label><br>
        <select id="fromDrop" class="btn btn-primary dropdown-toggle fromDrop">
        </select>
        <br>
        <br>
        <label>To :</label><br>
        <select id="toDrop" class="btn btn-primary dropdown-toggle toDrop"></select>
        <br>
        <br>
         <div id="distanceOf2Points"></div>
    </div>
</div>
</body>
...
$(function () {
    ...
    var fromToDropDown = '';
    for (var i = 0; i < indiaPorts.data.length; i++) {
        string += '<option value='+i+'>'+indiaPorts.data[i].state+'</option>';
        for (var j = 0; j < indiaPorts.data[i].statePorts.length; j++) {
            fromToDropDown += '<option value='+j+'-'+i+'>'+indiaPorts.data[i].statePorts[j].name+'</option>';
        }
    }
    $(document).find('.toDrop').html(fromToDropDown);
    $(document).find('.fromDrop').html(fromToDropDown);
    $(document).find('.optionsForStates').html(string);
    $(document).on('change', '.optionsForStates', function () {
      var optionText = $("#selectDrop option:selected").text();
      if(optionText == "All"){
        getGraphData(AllPorts, data);
      }
     });
   ...
   var distanceBWports = 0 ;

    getGraphData(AllPorts, data);
    // Initiate the chart
    function getGraphData (AllPorts, data) {
      // debugger;
        if (AllPorts.data.length == 2) {
            var distanceStatus = 'M'+' '+AllPorts.data[0].x+ ' ' +AllPorts.data[0].y+ ' '+'L'+ ' ' +AllPorts.data[1].x+ ' ' +AllPorts.data[1].y;
            var distaceBW2Points = getDistanceFromLatLonInKm(AllPorts);
        } else {
            var distanceStatus = 'M 0 L 0';
            var distaceBW2Points = 0;
  }
        distanceOf2Points = Math.round(distaceBW2Points);
        $('#cont').highcharts('Map', {
           chart: {
              map: 'countries/in/custom/in-all-disputed'
          },
            title: {
                text: 'Airports in India'
            },

            subtitle: {
                /*text: 'Source map: <a href="http://code.highcharts.com/mapdata/countries/in/in-all.js">India</a>'*/
                 // text: 'Source map: <a href="http://code.highcharts.com/mapdata/countries/in/custom/in-all-disputed.js">India with Airports</a>'
            },
            ...
            {
              type: 'mapline',
              name: 'Distance '+Math.round(distaceBW2Points)+' KM',
              color: '#bc2409',
              data: [{
                name: 'line',
                path: distanceStatus,
     ...
     $('#distanceOf2Points').html('Distance : '+distanceOf2Points+'KM'); 
    }

    function getDistanceFromLatLonInKm(latAndLongValues) {
      var lat1 = latAndLongValues.data[0].lat;
      var lat2 = latAndLongValues.data[1].lat;
      var lon1 = latAndLongValues.data[0].long;
      var lon2 = latAndLongValues.data[1].long;
    ...
    $(document).on('change', '.optionsForStates', function () {
        if (indiaPorts.data[$(this).val()] == ' ') {
            getGraphData(AllPorts, data);            
        } else {
            var dataPorts = {
                type: 'mappoint',
                data: indiaPorts.data[$(this).val()].statePorts 
            }
            getGraphData(dataPorts, data);
        }
    });

    function callFunctionSelected () {
        var dataMainObj = [];
        var from = $('option:selected', '#fromDrop').attr('value');
        var to = $('option:selected', '#toDrop').attr('value');
        //FROM
        var from_dataSplit = from.split('-');
        var from_child_index = from_dataSplit[0];
        var from_parent_index = from_dataSplit[1];
        var dataObjFrom = indiaPorts.data[from_parent_index].statePorts[from_child_index];
        dataMainObj.push(dataObjFrom);
        ///TO
        var to_dataSplit = to.split('-');
        var to_child_index = to_dataSplit[0];
        var to_parent_index = to_dataSplit[1];
        var dataObjTo = indiaPorts.data[to_parent_index].statePorts[to_child_index];
        dataMainObj.push(dataObjTo);
        var dataPorts = {
            type: 'mappoint',
            data: dataMainObj
        }
    ...
    $(document).on('change', '.fromDrop', function () {
        callFunctionSelected();
    });

    $(document).on('change', '.toDrop', function () {
        callFunctionSelected();
    });
