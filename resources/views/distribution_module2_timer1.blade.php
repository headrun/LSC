...
function getQuestions(index) {
  ...
  var optC = AllQuestions.data[index].C;
  var hiddenClassC = optC == '' ? 'hidden' : '';
    var html = '';
    html += '<div class="container"><div class="row" style="margin-left: 3%">  <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8"> <div id="question-one" class="quiz-ans-container" style="padding-bottom: 60px;padding-top: 5px;"> <h2 style="text-align:-webkit-left;font-family: monospace;"><span style="margin-left: -44px;" id="qqq">Q' + (index + 2) + '. </span>' + AllQuestions.data[index].name +' </h2></div></div></div> <div class="row"><div class="quiz-choice" onclick="WrongAnswer(this)"><p>'+ AllQuestions.data[index].A + '</p></div> <div class="quiz-choice" onclick="CorrectAnswer()"><p>'+ AllQuestions.data[index].B + '</p></div><div class="quiz-choice'+hiddenClassC+'" onclick="WrongAnswer()"><p>'+ AllQuestions.data[index].C + ' </p></div><div id ="opt4" class="quiz-choice'+hiddenClassD+'" onclick="WrongAnswer()"><p>'+ AllQuestions.data[index].D + '</p></div></div><div><br><div style="padding-right: 12px;margin-top: 25px;"><a href="#ex1" rel="modal:open" class= "open"><button>Show Solutionn</button></a></div><div id="ex1" class="modal"><h1 style="border-bottom: 1px solid grey;">Solution</h1><br><h3>'+AllQuestions.data[index].E +'</h3><br><hr><center><a href="#" rel="modal:close" style="font-size: 20px;text-decoration: none;" id="clos">Close</a></center></div>';
      $('.card').html(html);
      indexId = index;
      previousQsn = index;
